...
2. Create a function that will return the sum of two integers. Make up your own data and test the function.
```{r practice-2}
addfun <- function(x, y){
  x + y
}
...
pizzafun <- function(x){
  if(x == "cheese"){
    print("meh")} else if(x == "vegan cheese"){
    print("gross")
      } else if(x == "pepperoni"){
        print("yum")
        } else if(x == "bacon pineapple"){
    print("yum")
          } else if(x == "veggie"){
            print("yum, as long as there are no mushrooms")
          } else{
              "sounds intriguing"
            }
}

pizzafun("vegan cheese")
```
...
4. Use `plotfun()` to plot two variables from the `penguins` dataset:
```{r use-plotfun}
plotfun("flipper_length_mm", "bill_length_mm", "Flipper length (mm)", "Bill length (mm)", "Penguin flipper length vs. bill length")
```

5. Write and run your own function to make customized plots with the `mtcars` dataset in the chunk below. Remove the `eval = FALSE` chunk option when you're done.
